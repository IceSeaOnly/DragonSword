...
public class ClassUtils {
    ...
    public static List<String> classUnderPackage(String pkg) {
        return getClassName(pkg, false);
    }
    ...
    private static List<String> getClassNameByJar(String jarPath,
        ...
        try (JarFile jarFile = new JarFile(jarFilePath);) {
            ...
            while (entrys.hasMoreElements()) {
                ...
                if (entryName.endsWith(".class") && entryName.contains("binghai/app")) {
                    int index = entryName.lastIndexOf("/");
                    myClassName.add(entryName.substring(index + 1, entryName.length()-6));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
